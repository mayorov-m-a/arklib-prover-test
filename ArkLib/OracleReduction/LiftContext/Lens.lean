...
def OracleStatement.Lens (OuterStmtIn OuterStmtOut InnerStmtIn InnerStmtOut : Type)
    {Outer_ιₛᵢ : Type} (OuterOStmtIn : Outer_ιₛᵢ → Type) (O₁ : ∀ i, OracleContext Unit (ReaderM <| OuterOStmtIn i))
    {Outer_ιₛₒ : Type} (OuterOStmtOut : Outer_ιₛₒ → Type) (O₂ : ∀ i, OracleContext Unit (ReaderM <|OuterOStmtOut i))
    {Inner_ιₛᵢ : Type} (InnerOStmtIn : Inner_ιₛᵢ → Type) (O₃ : ∀ i, OracleContext Unit (ReaderM <|InnerOStmtIn i))
    {Inner_ιₛₒ : Type} (InnerOStmtOut : Inner_ιₛₒ → Type) (O₄ : ∀ i, OracleContext Unit (ReaderM <|InnerOStmtOut i))
  :=
